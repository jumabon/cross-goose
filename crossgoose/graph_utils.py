"""
from https://github.com/RUB-Bioinf/VesselExpress/blob/9da4fa0d706ddbcdf8218d3bd41ccb8ce5c7a5bf/VesselExpress/modules/networkx_graph_from_array.py
"""
...
def _get_increments(config_number, dimensions):
    ...
    neighbor_values = [(config_number >> digit) &
                       0x01 for digit in range(3 ** dimensions - 1)]
...
def _remove_clique_edges(networkx_graph):
    ...
    if len(list(three_vertex_cliques)) != 0:
        combination_edges = [list(itertools.combinations(clique, 2))
                             for clique in three_vertex_cliques]
        ...
        for main_dim, item in enumerate(subgraph_edge_lengths):
            if len(set(item)) != 1:
                for sub_dim, length in enumerate(item):
                    if length == max(item):
                        clique_edges.append(
                            combination_edges[main_dim][sub_dim])
            else:
                ...
                for num_spcl_edges in range(0, 3):
                    ...
                    diff_of_edges.append(
                        [i - j for i, j in zip(source, target)])
